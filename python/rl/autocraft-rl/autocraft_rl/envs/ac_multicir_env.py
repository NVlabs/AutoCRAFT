...
class AcRouteMultiCirEnv(gym.Env):
    ''' autocraft rl env '''
    ...
    # def __init__(self, cfg):
        ...
        self.observation_space = gym.spaces.Dict({
            ...
            'feats':       gym.spaces.Box(0, 10, shape=(self.max_segs, self.seg_feat_dim), dtype=np.float32),
        ...
        # self.valid_nets_cyc = cycle(self.valid_nets)
    ...
    def reset(self,
        ...
        if options is not None:
            if 'cir' in options:
                ...
                if 'net' in options and options['net'] != self.net.name():
    ...
    def get_observation(self, feats, edges, valid_nodes, valid_edges):
        ...
        # print(self.action_mask.dtype)
        # print(feats.dtype)
        # print(edges.dtype)
        # print(num_edges.dtype)
        return observation 

    def get_reward(self):
        # return 0
        return self.cfg.rl_reward_constant + \
               self.cfg.rl_reward_weight_wl  * (self.wl_pre - self.wl_cur) / self.net.bbox().hpwl() + \
               self.cfg.rl_reward_weight_via * (self.via_pre - self.via_cur) / self.cfg.rl_via_norm_constant + \
...
class AcRouteMultiCirFakeEnv(gym.Env):
