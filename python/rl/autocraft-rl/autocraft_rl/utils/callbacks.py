...
class AcRouteCallbacks(DefaultCallbacks):
    ...
    def __init__(self,
        ...
        # self.logdir_gds = './gds/'
        # self.logdir_gds_start = './gds/episode_start'
        # self.logdir_gds_end = './gds/episode_end'
        # self.logdir_topo = './topo/'
        # os.makedirs(self.logdir_gds, exist_ok=True)
        # os.makedirs(self.logdir_gds_start, exist_ok=True)
        # os.makedirs(self.logdir_gds_end, exist_ok=True)
        # os.makedirs(self.logdir_topo, exist_ok=True)
    ...
    def on_episode_end(self,
        ...
        episode.custom_metrics['wl_improvements'] = episode.user_data['wl_start'] - wl
        episode.custom_metrics['via_improvements'] = episode.user_data['via_start'] - via
        episode.custom_metrics['drv_improvements'] = episode.user_data['drv_start'] - drv
       
        if env.cfg.collect_topo:
            # print(yaml.dump(self.topo_dict, sort_keys=True, default_flow_style=False))
            # with open('topo_dict.json', 'w', encoding='utf-8') as f:
                # json.dump(self.topo_dict, f, ensure_ascii=False, indent=4)
